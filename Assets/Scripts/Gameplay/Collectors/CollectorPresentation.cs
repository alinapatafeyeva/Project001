...
{
    /// <summary>
    /// Decides which of this collector's two sprites (back-idle, front-idle)
    /// is currently shown, and asks CollectorView to apply it. Holds no
    /// rendering logic itself — CollectorView owns the SpriteRenderer and is
    ...
    ///
    /// Supports exactly two poses today: back-idle (shown only while queued
    /// on CollectorQueueBoard) and front-idle (shown from the moment the
    ...
    public class CollectorPresentation : MonoBehaviour
    {
        ...
        /// leaves its source — CollectorQueueBoard, Waiting Line, or Recovery
        /// Row. The collector stays front-facing for the rest of its
        /// lifecycle (Waiting Line, Recovery Row) — nothing switches it back
        /// to ShowQueueBack() afterwards.
        /// </summary>
